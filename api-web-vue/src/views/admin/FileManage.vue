    ...
    <el-card class="manage-card">
      <template #header>
        <div class="card-header">
          <span>文件管理</span>
          <el-button type="primary" @click="$router.push('/admin/file-upload')">
      ...
      >
        ...
        <el-table-column label="操作" width="320" fixed="right">
          <template #default="{ row }">
            ...
            <el-button link type="primary" @click="openRenameDialog(row)">
              重命名
            </el-button>
    ...
    <el-dialog v-model="renameVisible" title="重命名文件" width="400px">
      <el-form :model="renameForm" label-width="100px">
        <el-form-item label="原文件名">
          <el-input v-model="currentFileName" disabled />
        </el-form-item>
        <el-form-item label="新文件名" required>
          <el-input v-model="renameForm.newFileName" placeholder="请输入新文件名（保留扩展名）" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import { Upload, VideoCamera, Document } from '@element-plus/icons-vue'
...
const currentRow = ref<FileInfo | null>(null)
...
const loadFiles = async () => {
  ...
  try {
    const res = await fileApi.listFiles({
      ...
      prefix: filterForm.prefix || undefined
    })
...
// 获取文件类型名称
const getFileTypeName = (fileType?: string ,fileUrl?: string) => {
  // 优先从 URL 扩展名判断
  if (fileUrl) {
  ...
  if (fileType.includes('image')) {
    ...
    return ext ? ext.toUpperCase() : '图片'
  }
  if (fileType.includes('video')) {
    const ext = fileType.split('/')[1]
    return ext ? ext.toUpperCase() : '视频'
  }
...
const previewFile = (url: string) => {
  // 如果是图片、视频或 PDF，打开预览对话框
  if (isImage(url) || isVideo(url) || isPdf(url)) {
    previewUrl.value = url
    previewVisible.value = true
  } else {
    // 其他类型（如 DOCX、PPTX）直接下载
    downloadFile(url)
  }
}
...
// 打开重命名对话框
const openRenameDialog = (row: FileInfo) => {
  currentRow.value = row
  renameForm.oldObjectName = row.objectName || ''
  // 新文件名输入框显示原文件名（不含路径），让用户修改
  const fileName = row.fileName || extractFileName(row.objectName || '')
  currentFileName.value = fileName
  const houzhui = fileName.lastIndexOf('.')
  const extensionWithDot = houzhui === -1 ? '' : fileName.substring(houzhui);
  renameForm.newFileName = extensionWithDot|| ''
  renameVisible.value = true
}

// 从对象路径提取文件名
const extractFileName = (objectName: string) => {
...
const submitRename = async () => {
  if (!renameForm.newFileName || renameForm.newFileName.trim().length === 0) {
  ...
  try {
    await fileApi.renameFile(renameForm.oldObjectName, renameForm.newFileName)
...
const deleteFile = async (row: FileInfo) => {
  try {
    ...
    // 调用删除接口（同时删除 MinIO 和数据库记录）
    await fileApi.deleteFile(row.fileUrl!)
